...
func (env *environment) register(c *cobra.Command) {
	// TODO add env.envar and env.templateVars
	c.Flags().StringVar(&env.envFile, "env-file", "", "The path to a file with environment variable mappings of the form `NAME=value`. Template syntax can be used to inject secrets.")
	c.Flags().StringVar(&env.envFile, "template", "", "")
	_ = c.Flags().MarkHidden("template")
	c.Flags().StringVar(&env.templateVersion, "template-version", "auto", "The template syntax version to be used. The options are v1, v2, latest or auto to automatically detect the version.")
	c.Flags().BoolVar(&env.dontPromptMissingTemplateVar, "no-prompt", false, "Do not prompt when a template variable is missing and return an error instead.")
	c.Flags().StringVar(&env.secretsDir, "secrets-dir", "", "Recursively include all secrets from a directory. Environment variable names are derived from the path of the secret: `/` are replaced with `_` and the name is uppercased.")
	c.Flags().StringVar(&env.secretsEnvDir, "env", "default", "The name of the environment prepared by the set command (default is `default`)")
	_ = c.Flags().MarkHidden("env")
}
