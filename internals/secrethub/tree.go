...
func (cmd *TreeCommand) Register(c *cobra.Command) {
	command := &cobra.Command{
		Use:               "tree",
		Short:             "List contents of a directory in a tree-like format.",
		Args:              cobra.MaximumNArgs(1),
		ValidArgsFunction: AutoCompleter{client: GetClient()}.DirectorySuggestions,
		RunE:              cmd.Run,
	}

	command.Flags().BoolVarP(&cmd.fullPaths, "full-paths", "f", false, "Print the full path of each directory and secret.")
	command.Flags().BoolVarP(&cmd.noIndentation, "no-indentation", "i", false, "Don't print indentation lines.")
	command.Flags().BoolVar(&cmd.noReport, "no-report", false, "Turn off secret/directory count at end of tree listing.")
	command.Flags().BoolVar(&cmd.noReport, "noreport", false, "Turn off secret/directory count at end of tree listing.")
	//command.Flags().MarkHidden("noreport")

	c.AddCommand(command)
}

// Run prints the contents of a directory at a given path in a tree-like format.
func (cmd *TreeCommand) Run(c *cobra.Command, args []string) error {
	...
	t, err := client.Dirs().GetTree(args[0], -1, false)
	if err != nil {
		return err
	}
