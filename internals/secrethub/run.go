...
func (cmd *RunCommand) Register(c *cobra.Command) {
	...
	command.Flags().BoolVar(&cmd.noMasking, "no-masking", false, "Disable masking of secrets on stdout and stderr")
	command.Flags().BoolVar(&cmd.maskerOptions.DisableBuffer, "no-output-buffering", false, "Disable output buffering. This increases output responsiveness, but decreases the probability that secrets get masked.")
	command.Flags().DurationVar(&cmd.maskerOptions.BufferDelay, "masking-buffer-period", 50*time.Millisecond, "The time period for which output is buffered. A higher value increases the probability that secrets get masked but decreases output responsiveness.")
	command.Flags().BoolVar(&cmd.ignoreMissingSecrets, "ignore-missing-secrets", false, "Do not return an error when a secret does not exist and use an empty value instead.")
	cmd.environment.register(c)
