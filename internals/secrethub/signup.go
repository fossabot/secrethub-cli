...
func (cmd *SignUpCommand) Register(c *cobra.Command) {
	command := &cobra.Command{
		Use:   "signup",
		Short: "Create a free personal developer account.",
		RunE:  cmd.Run,
	}

	command.Flags().StringVar(&cmd.username, "username", "", "The username you would like to use on SecretHub.")
	command.Flags().StringVar(&cmd.fullName, "full-name", "", "Your full name.")
	command.Flags().StringVar(&cmd.email, "email", "", "Your (work) email address we will use for all correspondence.")
	command.Flags().StringVar(&cmd.org, "org", "", "The name of your organization.")
	command.Flags().StringVar(&cmd.orgDescription, "org-description", "", "A description (max 144 chars) for your organization so others will recognize it.")
...
func (cmd *SignUpCommand) Run(_ *cobra.Command, args []string) error {
