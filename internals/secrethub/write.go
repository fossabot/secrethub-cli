...
func (cmd *WriteCommand) Register(c *cobra.Command) {
	...
	command.Flags().BoolVarP(&cmd.useClipboard, "clip", "c", false, "Use clipboard as input.")
	command.Flags().BoolVarP(&cmd.multiline, "multiline", "m", false, "Prompt for multiple lines of input, until an EOF is reached. On Linux/Mac, press CTRL-D to end input. On Windows, press CTRL-Z and then ENTER to end input.")
	command.Flags().BoolVar(&cmd.noTrim, "no-trim", false, "Do not trim leading and trailing whitespace in the secret.")
	command.Flags().StringVarP(&cmd.inFile, "in-file", "i", "", "Use the contents of this file as the value of the secret.")
	c.AddCommand(command)
}

// Run handles the command with the options as specified in the command.
func (cmd *WriteCommand) Run(_ *cobra.Command, args []string) error {
	var err error
	...
	version, err := client.Secrets().Write(args[0], data)
	if err != nil {
		return err
	}
